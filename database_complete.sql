...
-- Provider Income (tracking provider earnings from verified payments)
CREATE TABLE IF NOT EXISTS provider_income (
    id INT PRIMARY KEY AUTO_INCREMENT,
    provider_id INT NOT NULL,
    payment_id INT NOT NULL,
    booking_id INT NOT NULL,
    total_amount DECIMAL(10,2) NOT NULL,
    platform_fee DECIMAL(10,2) NOT NULL, -- 20% of total amount
    provider_earnings DECIMAL(10,2) NOT NULL, -- 80% of total amount
...
-- Platform Revenue (tracking platform earnings)
...
CREATE INDEX idx_provider_income_provider ON provider_income(provider_id, status);
CREATE INDEX idx_provider_income_payment ON provider_income(payment_id);
CREATE INDEX idx_provider_income_created ON provider_income(created_at);
...
CREATE TABLE notifications (
    ...
    type ENUM('booking_approved', 'booking_confirmed', 'booking_completed', 'booking_cancelled', 'review_received', 'payment_required', 'payment_pending', 'payment_submitted', 'payment_verified', 'payment_rejected', 'general') DEFAULT 'general',
...
CREATE TABLE IF NOT EXISTS admin_notifications (
    ...
    title VARCHAR(255) NOT NULL,
    message TEXT NOT NULL,
    type ENUM('new_booking', 'payment_submitted', 'booking_completed', 'booking_cancelled', 'general') DEFAULT 'general',
    is_read BOOLEAN DEFAULT FALSE,
    related_id INT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_admin_notifications_admin ON admin_notifications(admin_id, is_read);
CREATE INDEX idx_admin_notifications_type ON admin_notifications(type, created_at);
