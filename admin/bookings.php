...
if (isset($_POST['action']) && isset($_POST['booking_id'])) {
    ...
    if ($action === 'confirm') {
        ...
        if ($booking) {
            ...
            // Send notification to admin about booking confirmation
            createNotification(
                $admin['id'], 
                'admin', 
                'Booking Confirmed', 
                "Booking #{$bookingId} confirmed: {$booking['customer_name']} → {$booking['provider_name']}", 
                'booking_confirmed', 
                $bookingId
            );
        } else {
        ...
        $booking = fetchOne("
            SELECT b.*, u.name as customer_name, sp.name as provider_name, sp.hourly_rate
            FROM bookings b 
            JOIN users u ON b.customer_id = u.id 
            JOIN service_providers sp ON b.provider_id = sp.id 
            WHERE b.id = ?
        ...
        if ($booking) {
            ...
            // Calculate payment amount (you can modify this logic based on your pricing structure)
            $paymentAmount = $booking['hourly_rate'] ?? 300; // Default to 300 if no hourly rate set
            ...
            if (!$existingPayment) {
                ...
                executeQuery("INSERT INTO payments (booking_id, customer_id, provider_id, amount, method, status) VALUES (?, ?, ?, ?, 'pending', 'pending')", [
                    $bookingId, $booking['customer_id'], $booking['provider_id'], $paymentAmount
                ]);
                ...
                createNotification(
                    ...
                    'customer', 
                    'Payment Required', 
                    "Your booking #{$bookingId} has been completed. Please submit your payment of ৳{$paymentAmount}.", 
                    'payment_required', 
                    $bookingId
                );
                ...
                createNotification(
                    ...
                    'provider', 
                    'Payment Pending', 
                    "Booking #{$bookingId} completed. Payment of ৳{$paymentAmount} is pending from customer.", 
                    'payment_pending', 
                    $bookingId
                );
            }
            
            setFlashMessage('success', 'Booking marked as completed. Payment system activated.');
            
            // Send notification to admin about booking completion
            createNotification(
                $admin['id'], 
                'admin', 
                'Booking Completed', 
                "Booking #{$bookingId} completed: {$booking['customer_name']} → {$booking['provider_name']}. Payment: ৳{$paymentAmount}", 
                'booking_completed', 
                $bookingId
            );
        } else {
        ...
        // Send notification to admin about booking cancellation
        if ($booking) {
            createNotification(
                $admin['id'], 
                'admin', 
                'Booking Cancelled', 
                "Booking #{$bookingId} cancelled: {$booking['customer_name']} → {$booking['provider_name']}", 
                'booking_cancelled', 
                $bookingId
            );
        }
	...
	if ($selectionAction === 'reject') {
		...
		if ($selection) {
			...
			createNotification(
				...
				'admin', 
				'New Booking Created', 
				"New booking created: {$selection['customer_name']} → {$selection['provider_name']} for {$selection['service_type']}", 
				'new_booking', 
				$bookingId
			);
...
<head>
    ...
    <title><?php echo $currentLang === 'en' ? 'Manage Bookings' : 'বুকিং পরিচালনা'; ?> - Home Service Admin</title>
        ...
        <nav class="container mx-auto px-4 py-4">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <a href="users.php" class="nav-link <?php echo basename($_SERVER['PHP_SELF']) == 'users.php' ? 'text-primary' : 'text-gray-600 hover:text-primary'; ?> transition-all font-bold">
                        ...
                        <div class="glass-card p-6 border-l-4 border-blue-500">
                            ...
                            <div class="mt-6 flex flex-wrap gap-3">
                                <form method="POST" onsubmit="return confirm('<?php echo $currentLang === 'en' ? 'Approve this selection?' : 'এই নির্বাচন অনুমোদন করবেন?'; ?>')">
                                    ...
                                    <button type="submit" class="flex items-center px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 hover:-translate-y-1 transition-all font-black text-xs uppercase tracking-wider">
                                        <i class="fas fa-check-circle mr-2"></i><?php echo $currentLang === 'en' ? 'Approve' : 'অনুমোদন'; ?>
                                    </button>
                                </form>
                                <a href="edit_selection.php?id=<?php echo $selection['id']; ?>" class="flex items-center px-5 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-xl shadow-sm hover:border-primary hover:text-primary hover:-translate-y-1 transition-all font-black text-xs uppercase tracking-wider">
                                ...
                                <form method="POST" onsubmit="return confirm('<?php echo $currentLang === 'en' ? 'Reject this selection?' : 'এই নির্বাচন প্রত্যাখ্যান করবেন?'; ?>')">
                                    ...
                                    <button type="submit" class="flex items-center px-5 py-2.5 bg-white border border-rose-200 text-rose-500 rounded-xl shadow-sm hover:bg-rose-50 hover:border-rose-300 hover:-translate-y-1 transition-all font-black text-xs uppercase tracking-wider">
                                        <i class="fas fa-times-circle mr-2"></i><?php echo $currentLang === 'en' ? 'Reject' : 'প্রত্যাখ্যান'; ?>
                                    </button>
                                </form>
                            </div>
                        </div>
